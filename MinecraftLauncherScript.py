...
def load_launcher_config():
    if not os.path.exists(LAUNCHER_CONFIG_PATH):
        try:
            ...
            print(f"Warning: cannot create {LAUNCHER_CONFIG_PATH}: {e}")
    ...
    try:
        ...
        print(f"Warning: cannot load {LAUNCHER_CONFIG_PATH}: {e}")
        return DEFAULT_LAUNCHER_CONFIG
...
# Категории: {UI-имя: (локальная_папка_внутри_инстанса, облачная_папка)}
CATEGORIES = {
    "Моды (mods)": ("mods", "mods"),
...
class UpdaterApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Папка, откуда запущен скрипт/exe
        ...
        # Загружаем настройки (ссылка, ник, выбранные категории) – root_dir больше не используется
        (self.public_url,
         self.nickname,
    ...
    def get_minecraft_dir(self):
        """Папка инстанса Sex3: base_path/Minecraft/instances/Sex3"""
        return os.path.join(self.base_path, "Minecraft", "instances", "Sex3")

    def get_instance_parent_dir(self):
        """Папка, в которой cmd-launcher хранит инстансы: base_path/Minecraft"""
    ...
    def load_or_create_config(self):
        ...
        # Если отсутствуют public_url или nickname – диалог
        if "public_url" not in config or "nickname" not in config:
            ...
            self.save_config(
                ...
                config.get("selected_categories", [local for local, _ in CATEGORIES.values()])
            )
            return (config_dialog["public_url"],
                    config_dialog["nickname"],
    ...
    def save_config(self, public_url, nickname, selected_categories=None):
        config = {
    ...
    def request_config_dialog(self, current_url, current_nick):
        ...
        entry_url.grid(row=1, column=0, padx=(10, 5), pady=5, sticky="we")
        ...
        entry_nick.grid(row=3, column=0, padx=(10, 5), pady=5, sticky="we")
        ...
        frame_btns.grid(row=4, column=0, columnspan=2, pady=15)
    ...
    def launch_minecraft(self):
        ...
        if self.is_instance_installed():
            ...
            if answer:
                ...
                # Раньше открывались настройки; сейчас просто разблокируем кнопку
                # (можно и вовсе не открывать, т.к. папка всегда одна)
                pass
    ...
    def _install_and_launch(self):
        ...
        all_local_paths = [local for local, _ in CATEGORIES.values()]
        success, _, _ = self.sync_process(all_local_paths, silent=True)
        if not success:
    ...
    def download_folder_as_zip(self, public_key, folder_name, dest_file):
        """
        Скачивает папку folder_name как ZIP через API Яндекс.Диска.
        """
        params = {
            "public_key": public_key,
            "path": f"/{folder_name}"
        }
        self.after(0, self.log_message, f"[СЕТЬ] Запрос ссылки на ZIP папки {folder_name}")
        try:
            resp = requests.get(
            ...
            if resp.status_code != 200:
                self.after(0, self.log_error, "Скачивание", f"Ошибка получения ссылки: HTTP {resp.status_code}")
                return False

            data = resp.json()
            download_url = data.get("href")
            if not download_url:
                self.after(0, self.log_error, "Скачивание", "Не получен href для ZIP")
                return False

            self.after(0, self.log_message, "[СЕТЬ] Скачивание ZIP-архива...")
            dl_resp = requests.get(download_url, headers=HEADERS, stream=True, timeout=120)
            if dl_resp.status_code == 200:
                os.makedirs(os.path.dirname(dest_file), exist_ok=True)
                with open(dest_file, "wb") as f:
                    for chunk in dl_resp.iter_content(chunk_size=8192):
                        f.write(chunk)
                self.after(0, self.log_message, f"[УСПЕХ] config.zip сохранён ({os.path.getsize(dest_file)} байт)")
                return True
            else:
                self.after(0, self.log_error, "Скачивание", f"Ошибка загрузки ZIP: HTTP {dl_resp.status_code}")
                return False
        except Exception as e:
            self.after(0, self.log_error, "Скачивание", f"Ошибка: {e}")
            return False

    def sync_config_zip(self, silent=False):
        """Проверяет наличие config.zip в корне инстанса и скачивает при отсутствии."""
        minecraft_dir = self.get_minecraft_dir()
        zip_path = os.path.join(minecraft_dir, "config.zip")
        if os.path.isfile(zip_path):
            self.after(0, self.log_message, "[КОНФИГ] config.zip уже существует, пропускаем.")
            return True, 0, 0
        self.after(0, self.log_message, "[КОНФИГ] config.zip не найден, скачиваем архив конфигов с Яндекс.Диска...")
        if self.download_folder_as_zip(self.public_key, "config", zip_path):
            return True, 1, 0
        else:
            return False, 0, 0
    ...
    def sync_process(self, selected_local_paths, silent=False):
        ...
        # Отдельная обработка config (ZIP)
        if "config" in selected_local_paths:
            success, dl, _ = self.sync_config_zip(silent)
            if success:
                downloaded += dl
            else:
                has_errors = True
            selected_local_paths = [p for p in selected_local_paths if p != "config"]

        # Остальные категории (mods) – обычная синхронизация
        if selected_local_paths:
            ...
            for local_path in selected_local_paths:
                ...
                for cloud_rel_path, (download_url, cloud_size) in cloud_files.items():
                    if cloud_rel_path.startswith(cloud_folder + "/"):
                        ...
                        local_rel = local_path + "/" + os.path.basename(cloud_rel_path)
        ...
        if has_errors:
            self.after(0, self.log_message, f"\n⚠️ [ЗАВЕРШЕНО С ОШИБКАМИ] Загружено файлов: {downloaded}.")
            if not silent:
            ...
            if not silent:
                self.after(0, lambda: messagebox.showinfo("Готово", "Моды и конфиги успешно обновлены!"))
            return True, downloaded, deleted
...
if __name__ == "__main__":
    if "--headless" in sys.argv:
        ...
        app.log_message = lambda msg: print(msg)
